...
(function( $, window, document, undefined ) {
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sInfo": "Gösterilen _TOTAL_ Kayıt",
			 ...
			 *    } );
			 */
			"sInfoFiltered": "(Seçilen from _MAX_ total Kayıt)",
	...
	$.extend( extPagination, {
		...
		simple_numbers: function ( page, pages ) {
		    return ['Geri', _numbers(page, pages), 'İleri'];
		},

		full_numbers: function ( page, pages ) {
		    return ['İlk', 'Geri', _numbers(page, pages), 'İleri', 'Son'];
		},
					...
					for ( i=0, ien=buttons.length ; i<ien ; i++ ) {
						...
						if ( $.isArray( button ) ) {
							...
							switch ( button ) {
								...
								case 'Birinci':
								    btnDisplay = lang.sBirinci;
									btnClass = button + (page > 0 ?
										'' : ' '+classes.sPageButtonDisabled);
									break;

								case 'Geri':
								    btnDisplay = lang.sGeri;
									btnClass = button + (page > 0 ?
										'' : ' '+classes.sPageButtonDisabled);
									break;

								case 'İleri':
								    btnDisplay = lang.sİleri;
									btnClass = button + (page < pages-1 ?
										'' : ' '+classes.sPageButtonDisabled);
									break;

								case 'Son':
									btnDisplay = lang.sSon;
									btnClass = button + (page < pages-1 ?
										'' : ' '+classes.sPageButtonDisabled);
									break;
